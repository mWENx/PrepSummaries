...
class DocxService {
  ...
  /// [additionalNotes] — optional freeform text appended at the end of the document
    ...
    for (final file in archive) {
      ...
      if (_isWordXml(file.name)) {
        ...
        final modified = _processXml(
          ...
          additionalNotes:
              file.name == 'word/document.xml' ? additionalNotes : null,
        );
  ...
  static String _processXml(
    ...
    // 5. Append additional notes at the end of the document
    if (additionalNotes != null && additionalNotes.isNotEmpty) {
      _appendAdditionalNotes(doc, additionalNotes);
    }
  ...
  // ── Additional notes ──────────────────────────────────────────────────────
  //
  // Appends an "ADDITIONAL NOTES" heading and body paragraph before the
  // sectPr at the end of the document body.

  static void _appendAdditionalNotes(XmlDocument doc, String notes) {
    final body = doc.findAllElements('body', namespace: _wNS).firstOrNull;
    if (body == null) return;

    // Find the sectPr (last element of body) to insert before it
    final children = body.children;
    int insertIdx = children.length;
    for (int i = children.length - 1; i >= 0; i--) {
      final child = children[i];
      if (child is XmlElement && child.name.local == 'sectPr') {
        insertIdx = i;
        break;
      }
    }

    // Build heading paragraph ("ADDITIONAL NOTES")
    final headingXml = XmlDocument.parse(
      '<?xml version="1.0"?>'
      '<root xmlns:w="$_wNS" xml:space="preserve">'
      '<w:p>'
      '<w:pPr><w:spacing w:before="360" w:after="80"/></w:pPr>'
      '<w:r><w:rPr><w:b/><w:bCs/></w:rPr>'
      '<w:t xml:space="preserve">ADDITIONAL NOTES</w:t></w:r>'
      '</w:p>'
      '</root>',
    );
    children.insert(
        insertIdx, headingXml.rootElement.childElements.first.copy());
    insertIdx++;

    // Build body paragraph with the notes text
    final escaped = _escapeXml(notes);
    final bodyXml = XmlDocument.parse(
      '<?xml version="1.0"?>'
      '<root xmlns:w="$_wNS" xml:space="preserve">'
      '<w:p>'
    ...
    children.insert(
        insertIdx, bodyXml.rootElement.childElements.first.copy());
  }
