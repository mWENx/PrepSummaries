...
class ExcelService {
  /// Returns all unique donor names from [filePath], sorted alphabetically.
  static List<String> getAllDonorNames(String filePath) {
    ...
    for (int i = 0; i < sheet.rows[0].length; i++) {
      ...
      if (_cellStr(cell) == 'Constituent: First and Last Name') {
        nameColIdx = i;
        break;
      }
    }
    if (nameColIdx == null) return [];

    final names = <String>{};
    for (int i = 1; i < sheet.rows.length; i++) {
      final row = sheet.rows[i];
      final name =
          nameColIdx < row.length ? _cellStr(row[nameColIdx]) : '';
